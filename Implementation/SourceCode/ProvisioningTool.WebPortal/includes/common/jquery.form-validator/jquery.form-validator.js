...
(function ($) {
    ...
    */
    $.fn.validateInputOnBlur = function (language, conf, attachKeyupEvent, eventContext) {
        ...
        //    debugger;
        //    //alert($(this).ID);//or any process you want to be done
        //    _applyErrorStyle($elem, conf);
        //}

        if ($(this).val().trim().length == 0 && $(this).hasClass("custclass")) { 
            _applyErrorStyle($(this), conf);
        }
    ...
    $.fn.isValid = function (language, conf, displayError) {
        ...
        $("input").each(function () {
            if ($(this).val().trim().length == 0 && $(this).hasClass("custclass")) { 
                _applyErrorStyle($(this), conf);
              //  var iVal = $('#ddlCountry').val(); 
            }
        });

        $(".custddl").each(function () {
            
            if ($(this).val() === '0') {  
            //    $('.custddl').find('.select2-choice').removeClass('border');
                var id = '#s2id_' + this.id;

                $(id).addClass("errorBox");
                $(id).find('.select2-choice').css({ "background": "#ffebef" });
                $('.select2-container .select2-choice').css({ "border": "1px solid red !important" });
                $('.select2-container .select2-choice').css({ "border-color": "red" });
                //$('.custddl').find('.select2-choice').css({ "border-color": "red" });
            }
        });
    ...
    */
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction: function (tele) {
            ...
            return tele.length == 10 && tele.match(/[^0-9\s()-]/g) === null;
        },
        errorMessage: '',
